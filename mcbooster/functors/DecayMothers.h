...
struct DecayMothers {
    ...
    __host__ __device__ inline GReal_t process(const GInt_t evt, Vector4R **particles) {
...
#pragma unroll(9)
        ...
        particles[1]->set(sqrt(pd[0] * pd[0] + fMasses[0] * fMasses[0]), 0.0, pd[0], 0.0);

#pragma unroll(9)
        for(size_t i = 1; i < fNDaughters; i++) {
            particles[i + 1]->set(sqrt(pd[i - 1] * pd[i - 1] + fMasses[i] * fMasses[i]), 0.0, -pd[i - 1], 0.0);
